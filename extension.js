...
class NanoBotsDispatcher {
  static async run(params, counter) {
    ...
    if (!editor) {
      vscode.window.showErrorMessage('Nano Bots: No open file. Open one to proceed.');
      return;
    }
  ...
  static async ask_for_cartridge(params, counter) {
    ...
    vscode.window.showQuickPick(options, { placeHolder: 'Select a Cartridge' }).then((selected) => {
      if (selected) {
  ...
  static ask_for_input(params, counter) {
    vscode.window.showInputBox({ prompt: 'Prompt' }).then((input) => {
      if (input) {
        params.input = input;
        NanoBotsDispatcher.run(params, counter + 1);
      }
